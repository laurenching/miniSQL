...
{
	record.deleteTableFile(table_name);
...
Table API::unionTable(Table &table1, Table &table2, std::string target_attr, Where where)
{
	...
	int i;
	Attribute attr = table1.getAttribute();
	for (i = 0; i < 32; i++)
	...
	//            switch (data1[i].type) {
	//                case -1: {
	//                    if (data1[i].intData != data2[i].intData) {
	//                        if (data1[i].intData < data2[i].intData)
	//                            flag = 1;
	//                        else
	//                            flag = -1;
	...
	//                } break;
	//                case 0: {
	//                    if (data1[i].floatData != data2[i].floatData) {
	//                        if (data1[i].floatData < data2[i].floatData)
	//                            flag = 1;
	//                        else
	//                            flag = -1;
	...
	//                } break;
	//                default: {
	//                    if (data1[i].stringData < data2[i].stringData)
	//                        flag = 1;
	//                    else
	//                        flag = -1;
	//                } break;
...
bool sortcmp(const Tuple &tuple1, const Tuple &tuple2)
{
	...
	switch (data1[0].type) {
	case -1: return data1[0].intData < data2[0].intData;
	case 0: return data1[0].floatData < data2[0].floatData;
	default: return data1[0].stringData < data2[0].stringData;
	}
}
...
bool calcmp(const Tuple &tuple1, const Tuple &tuple2)
{
	...
	for (i = 0; i < data1.size(); i++) {
		...
		switch (data1[0].type) {
		case -1: {
			if (data1[0].intData != data2[0].intData)
				flag = true;
		}break;
		case 0: {
			if (data1[0].floatData != data2[0].floatData)
				flag = true;
		}break;
		default: {
			if (data1[0].stringData != data2[0].stringData)
				flag = true;
		}break;
		}
		if (flag)
...
bool isSatisfied(Tuple& tuple, int target_attr, Where where)
{
	...
	case Less: {
		...
		case -1: return (data[target_attr].intData < where.data.intData); break;
		case 0: return (data[target_attr].floatData < where.data.floatData); break;
		default: return (data[target_attr].stringData < where.data.stringData); break;
		}
	} break;
	case lessEqual: {
		switch (where.data.type) {
